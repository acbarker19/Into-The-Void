...
*/
function PageHitButtons(pageName, animationList, optionList)
{
	...
	{
		//alert("PageDropObjectInBasket.doMouseDown "+_pageName+"   "+point.x +", " +point.y);
		...
		for (i=0;i< _animationList.length; i++)
		{
			if (_animationList[i].hitTest(point))
			{
				if (_animationList[i].canDrag){
					...
					//alert("click");
					_numOrangesDropped++;
					this.userSelectedObject.notClickable();
					if (_numOrangesDropped == 3)
					{
			
				
						//alert("You have dropped 3 oranges in the basket!");
	...
	{
/*		//alert("mouseup "+point.x +", " +point.y)
		if (this.userSelectedObject != null)
		{
			//alert("Just dropped an orange.");
			...
			if (nameOfLastObject == "Basket")
			{
				if (lastObject.hitTest(point))
				{
					//user can no longer drag the userSelectedObject
					this.userSelectedObject.canDrag = false;
					_numOrangesDropped++;
					if (_numOrangesDropped == 3)
					{
